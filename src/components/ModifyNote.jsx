...
const handleShortNoteSubmit = async () => {
    try {
      const updatedShortNote = {
        start_page: start,
        end_page: end,
      ...
      const response = await axiosInstance.patch(`/desk/books/${isbn}/note/short/${id}`, updatedShortNote, {
        headers: {
...
const handleLongNoteSubmit = async () => {
    try {
      const updatedLongNote = {
        start_page: start,
        end_page: end,
        read_complete: checked,
        long_title: title,
      ...
      const response = await axiosInstance.patch(`/desk/books/${isbn}/note/long/${id}`, updatedLongNote, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      ...
      {activeTab === 'detailed' && (
        <FormContainer>
          ...
          <OpenSet>
           ...
           <SubmitButton onClick={() => handleLongNoteSubmit(memberId, myBookId)}>수정하기</SubmitButton>
           </div>
        </FormContainer>
      )}
      </NoteContainer>
